...
export function validateContent(payload: Record<string, unknown>): ContentInput {
  const type = sanitizePlain(payload.type, 20) as ContentType; const status = sanitizePlain(payload.status, 20) as PublicationStatus; const title = sanitizePlain(payload.title, 240); const slug = slugify(sanitizePlain(payload.slug || title, 160));

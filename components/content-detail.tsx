...
import { getSettings, listContent } from "@/lib/database";
import { contentTypographyStyle, normalizeSiteSettings } from "@/lib/site-settings";
...
export async function ContentDetail({item,preview=false}:{item:ContentItem;preview?:boolean}){
  const [relatedItems,settings]=await Promise.all([listContent({publishedOnly:true,category:item.categorySlug??undefined,limit:4}),getSettings()]);
  const related=relatedItems.filter(v=>v.id!==item.id).slice(0,3);const site=normalizeSiteSettings(settings.site);
  ...
  return <main id="top" className="article-site" style={contentTypographyStyle(site)}><PublicHeader site={site}/>{preview&&<div className="preview-banner">DRAFT PREVIEW · Only visible in the admin</div>}<article className="article-page page-shell">
  ...
  </article>{related.length>0&&<section className="related page-shell"><div className="section-mark">CONTINUE EXPLORING</div><h2>Keep wandering.</h2><ContentList items={related}/></section>}<PublicFooter site={site}/></main>
}

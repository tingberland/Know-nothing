...
export async function ContentDetail({item,preview=false}:{item:ContentItem;preview?:boolean}){
  ...
  return <main id="top"><PublicHeader/>{preview&&<div className="preview-banner">DRAFT PREVIEW · Only visible in the admin</div>}<article className="article-page page-shell">
    ...
    {item.featuredImageUrl&&<figure className="article-hero-media">{item.featuredImageMime?.startsWith("video/")?<video src={item.featuredImageUrl} controls preload="metadata"/>:<Image src={item.featuredImageUrl} alt={item.featuredImageAlt??""} width={1600} height={900} sizes="(max-width: 850px) 100vw, 92vw" priority/>}</figure>}

...
export async function POST(request:Request){const auth=await requireApiUser(request);if("response"in auth)return auth.response;const p=await request.json() as Record<string,unknown>;const name=sanitizePlain(p.name,80);const slug=slugify(name);if(!name||!slug)return Response.json({error:"Enter a valid tag name."},{status:400});await saveTaxonomy("tags",{name,slug});return Response.json({tags:await listTags()})}
